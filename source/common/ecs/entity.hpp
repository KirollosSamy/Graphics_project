...
namespace our
{
    ...
    public:
        ...
        T *getComponent()
        {
            ...
            {
                if(T* comp = dynamic_cast<T*>(component))   // if component cannot be casted a Null pointer is returned.
                    return comp;
            }
            return nullptr;
        }
        ...
        void deleteComponent(T const *component)
        {
            ...
            //  If found, delete the found component and remove it from the components list
            auto it = components.find(components.begin(),components.end(),component)
             if (it != components.end())
            {
                delete *it;
                components.erase(it);
