...
// The sorting function takes two RenderCommand objects as input, referred to as "first" and "second". It then calculates the distance of each object to the camera using the dot product of the object's local-to-world matrix times the object's center position in homogeneous coordinates with the camera forward vector.
// Finally, the function returns a boolean value based on whether the distance of the first object is greater than the distance of the second object. If it is, then the sorting function will consider the first object to be "less than" the second object, which means it will appear earlier in the sorted vector. If it is not, then the second object will appear earlier in the sorted vector.
